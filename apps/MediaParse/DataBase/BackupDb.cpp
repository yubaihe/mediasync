...
BOOL CBackupTable::UpdateItemGpsAddr(string strGps, string strAddr)
{
    CDbDriver* pDbDriver = LOCKBACKUPDB
    BOOL bRet = pDbDriver->ExecuteSQL("update %s set location='%s' where weizhi='%s'", TABLE_BACKUP, strAddr.c_str(), strGps.c_str());
    UNLOCKBACKUPDB
    return bRet;
}
...
BOOL CBackupTable::UpdatePaiTimeSec(int iID, time_t iPaiTimeSec, string strMediaAddr)
{
    CDbDriver* pDbDriver = LOCKBACKUPDB
    BOOL bRet = pDbDriver->ExecuteSQL("update %s set paishetime='%d',mediaaddr='%s' where id='%d'", TABLE_BACKUP, iPaiTimeSec, strMediaAddr.c_str(), iID);
    UNLOCKBACKUPDB
    return bRet;
}
