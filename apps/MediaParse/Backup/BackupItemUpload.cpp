...
string CBackupItemUpload::GetNewFileName(string strOriginalFile, string strFold, MEDIATYPE eMediaType, string& strOutThumbFile, time_t iCreateTimeSec)
{
    ...
    string strExtension = CCommonUtil::GetFileExtension(strOriginalFile);
    ...
    do
    {
        ...
        if(iIndex == 0)
        {
            strFileName = CCommonUtil::StringFormat("%lld", iCreateTimeSec);
        }
        else
        {
            strFileName = CCommonUtil::StringFormat("%lld_%d", iCreateTimeSec, iIndex);
        }
        iIndex++;
        strFileName.append(strExtension);
...
BOOL CBackupItemUpload::Save()
{
    ...
    switch(m_eMediaType)
    {
        case MEDIATYPE_IMAGE:
        {
            ...
            item.iCreateTimeSec = CCommonUtil::TimeToSec(m_ImageParse.GetCreateTime());
            std::vector<std::string> vecLocation = m_ImageParse.GetLocaiton();
            item.strAddr = CCommonUtil::StringFormat("%s&%s", vecLocation[0].c_str(), vecLocation[1].c_str());
            item.iWidth = m_ImageParse.GetWidth();
            item.iHeight = m_ImageParse.GetHeight();
            item.iDuration = 0;
            break;
        }
        case MEDIATYPE_VIDEO:
        {
            ...
            item.iCreateTimeSec = CCommonUtil::TimeToSec(m_VideoParse.GetCreateTime());
            std::vector<std::string> vecLocation = m_VideoParse.GetLocaiton();
            item.strAddr = CCommonUtil::StringFormat("%s&%s", vecLocation[0].c_str(), vecLocation[1].c_str());
            item.iWidth = m_VideoParse.GetWidth();
            item.iHeight = m_VideoParse.GetHeight();
            item.iDuration = m_VideoParse.GetDurationSec();
            break;
        }
        default:
        {
...
BOOL CBackupItemUpload::Backup()
{
    ...
    if (TRUE == CCommonUtil::IsMimeTypeImage(strMimeType.c_str()))
    {
        ...
        m_ImageParse.Parse(m_strOriginalFile.c_str());
        time_t iCreateTimeSec = CCommonUtil::TimeToSec(m_ImageParse.GetCreateTime());
        strDestFile = GetNewFileName(m_strOriginalFile, m_strFoldName, MEDIATYPE_IMAGE, strDestThumbFile, iCreateTimeSec);
    }
    else if (TRUE == CCommonUtil::IsMimeTypeVideo(strMimeType.c_str()))
    {
        m_eMediaType = MEDIATYPE_VIDEO;
        m_VideoParse.Parse(m_strOriginalFile.c_str());
        time_t iCreateTimeSec = CCommonUtil::TimeToSec(m_VideoParse.GetCreateTime());
        strDestFile = GetNewFileName(m_strOriginalFile, m_strFoldName, MEDIATYPE_VIDEO, strDestThumbFile, iCreateTimeSec);
    }
