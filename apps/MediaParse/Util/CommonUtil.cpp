...
{
    size_t iLastSlash = strPathFile.find_last_of('/');
    std::string strFileName = (iLastSlash == std::string::npos) ? strPathFile : strPathFile.substr(iLastSlash + 1);

    // 2. 在文件名中找到最后一个 '.'
    size_t iLastDot = strFileName.find_last_of('.');
    if (iLastDot != std::string::npos && iLastDot != 0)
    {
        std::string strExtension = strFileName.substr(iLastDot);
        return strExtension;
    }
    else
    {
