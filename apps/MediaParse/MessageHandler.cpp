...
string CMessageHandler::OnParseGps(nlohmann::json& jsonRoot)
{
    ...
    if(imageParse.GetLatitude() != 0 && imageParse.GetLongtitude() != 0)
    {
        nlohmann::json jsonRet;
        jsonRet["status"] = 1;
        jsonRet["gps"] = CCommonUtil::StringFormat("%f&%f", imageParse.GetLongtitude(), imageParse.GetLatitude());
        return MediaParse::CJsonUtil::ToString(jsonRet);
    }
    ...
    if(videoParse.GetLatitude() != 0 && videoParse.GetLongtitude() != 0)
    {
        nlohmann::json jsonRet;
        jsonRet["status"] = 1;
        jsonRet["gps"] = CCommonUtil::StringFormat("%f&%f", videoParse.GetLongtitude(), videoParse.GetLatitude());
        return MediaParse::CJsonUtil::ToString(jsonRet);
    }
    nlohmann::json jsonRet;
    jsonRet["status"] = 0;
...
string CMessageHandler::OnBackupUploadItemAddrChange(nlohmann::json& jsonRoot)
{
    ...
    int iItemID = -1;
    if (TRUE == jsonRoot.contains("itemid"))
    {
        iItemID = jsonRoot["itemid"];
    }

    std::vector<std::string> vec = CCommonUtil::StringSplit(strGps, "&");
    if(vec.size() != 2)
    {
        nlohmann::json jsonRet;
    ...
    double dLongitude = atof(vec[0].c_str());
    double dLatitude = atof(vec[1].c_str());
    ...
    if(iItemID <= 0)
    {
        bRet = table.UpdateItemGpsAddr(strGps, strAddr);
    }
    else
    {
        bRet = table.UpdateItemGpsAddr(iItemID, strGps, strAddr);
    }
...
string CMessageHandler::OnBackupMediaTimeUpdate(nlohmann::json& jsonRoot)
{
    int iItemID = jsonRoot["id"];
    ...
    string strBackupOriginalFile = CBackupManager::GetInstance()->GetBackupRoot(item.strFoldName);
    string strOriginalFrom(strBackupOriginalFile);
    strOriginalFrom.append(item.strFile);

    string strBackupThumbFile = CBackupManager::GetInstance()->GetBackupThumbRoot(item.strFoldName);
    string strOriginalThumbFrom(strBackupThumbFile);
    string strTmp = CCommonUtil::ReplaceStr(item.strFile, ".", "_");
    strOriginalThumbFrom.append(strTmp);
    strOriginalThumbFrom.append(".jpg");

    string strDestFile;
    string strDestThumbFile;
    CBackupItemUpload itemUpload;
    strDestFile = itemUpload.GetNewFileName(strOriginalFrom, item.strFoldName, item.eMediaType, strDestThumbFile, iTimeSec);
    string strOriginalTo(strBackupOriginalFile);
    strOriginalTo.append(strDestFile);
    string strOriginalThumbTo(strBackupThumbFile);
    strOriginalThumbTo.append(strDestThumbFile);
    printf("strOriginalFrom:%s\nstrOriginalThumbFrom:%s\nstrOriginalTo:%s\nstrOriginalThumbTo:%s\n", strOriginalFrom.c_str(), strOriginalThumbFrom.c_str(), strOriginalTo.c_str(), strOriginalThumbTo.c_str());
    CCommonUtil::MoveFile(strOriginalFrom, strOriginalTo);
    CCommonUtil::MoveFile(strOriginalThumbFrom, strOriginalThumbTo);
    BOOL bRet = table.UpdatePaiTimeSec(iItemID, iTimeSec, strDestFile);
    nlohmann::json jsonRet;
    jsonRet["status"] = bRet;
    return MediaParse::CJsonUtil::ToString(jsonRet);
}
