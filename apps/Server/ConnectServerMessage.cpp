...
string CConnectServerMessage::OnDeviceInfo(nlohmann::json& jsonRoot)
{
    ...
    jsonRet["lastupdatetime"] = mediaItem.iAddTimeSec;
...
string CConnectServerMessage::OnReportInfo(nlohmann::json& jsonRoot)
{
	...
	{
		long iUpdateTimeSec = jsonRoot["utimesec"];
		Server::CTools::UpdateTimeSec(iUpdateTimeSec);
	}
	//文件都是对的 下面开始存储信息了
	//判断年份文件夹是否存在
	long iPaiSheTimeSec = jsonRoot["paitime"];
	if(iPaiSheTimeSec == 0)
	{
		//没有拍摄时间用当前时间
		iPaiSheTimeSec = Server::CTools::CurTimeSec();
	}
	int iYear = Server::CTools::GetYear(iPaiSheTimeSec);
	printf("iPaiSheTimeSec:%ld year:%d\n", iPaiSheTimeSec, iYear);
	...
	string strFileName = CFileUtil::GetNewFileName(iYear, iPaiSheTimeSec, strPostFix.c_str());
	...
	if(FALSE == Server::CTools::ParseGps(strWeiZhi, &dLong, &dLat))
	{
	...
	item.iPaiSheTimeSec = iPaiSheTimeSec; 							//拍摄时间
	item.iAddTimeSec = Server::CTools::CurTimeSec();               //添加时间
...
string CConnectServerMessage::OnUpdatePaiSheTime(nlohmann::json& jsonRoot)
{
    ...
    //get new filename.
    string strNewFileName = CFileUtil::GetNewFileName(iYear, iTimeSec, szPostFix);
    ...
    //修改封面
    CMediaCoverTable::ChangeCoverAddr(item.strAddr, strNewFile.c_str());
    //计数 -1
    CMediaJishuTable::Decrease(item.iPaiSheTimeSec, item.iMediaType, item.strDeviceName);
    //计数 +1
...
string CConnectServerMessage::OnThisDay(nlohmann::json& jsonRoot)
{
    ...
    for(; itor != YearList.end(); ++itor)
    {
        ...
        itemJson["time"] = mediaItem.iPaiSheTimeSec;
...
string CConnectServerMessage::OnUnCheckGps(nlohmann::json& jsonRoot)
{
    int iLimit = jsonRoot["limit"];
    CMediaInfoTable table;
    list<string> unCheckGpsList = table.GetUnCheckWeiZhi(iLimit);
    nlohmann::json jsonRetItems = nlohmann::json::array();
    for(list<string>::iterator itor = unCheckGpsList.begin(); itor != unCheckGpsList.end(); ++itor)
    {
        jsonRetItems.push_back(*itor);
    }
    nlohmann::json jsonRet;
    jsonRet["status"] = 1;
    jsonRet["items"] = jsonRetItems;
...
string CConnectServerMessage::OnUpdateGpsLocation(nlohmann::json& jsonRoot)
{
    ...
    std::vector<std::string> vec = Server::CCommonUtil::StringSplit(strGps, "&");
    if(vec.size() != 2)
    {
        nlohmann::json jsonRet;
    ...
    if(iItemID <= 0)
    {
        bRet = CMediaInfoTable::UpdateGpsWeiZhi(strGps, strAddr);
    }
    else
    {
        bRet = CMediaInfoTable::UpdateGpsWeiZhi(iItemID, strGps, strAddr);
    }
