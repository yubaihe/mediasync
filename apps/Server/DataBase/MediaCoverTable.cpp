...
BOOL CMediaCoverTable::ChangeCoverAddr(string strOldAddr, string strNewAddr)
{
    CDbDriver* pDbDriver = LOCKMEDIADB
    BOOL bRet = pDbDriver->ExecuteSQL("update tbl_mediacover set mediaaddr='%s' where mediaaddr='%s'", strNewAddr.c_str(), strOldAddr.c_str());
    UNLOCKMEDIADB
    return bRet;
}
