...
list<MediaInfoItem> CMediaInfoTable::AssembleItems(list<map<string, string>> List)
{
    ...
    for(; itor != List.end(); ++itor)
    {
        ...
        for(itorMap = mapItem.begin(); itorMap != mapItem.end(); ++itorMap)
        {
            ...
            {
                item.iPaiSheTimeSec = atol(strValue.c_str());
            }
            ...
            {
                item.iAddTimeSec = atol(strValue.c_str());
            }
...
MediaInfoItem CMediaInfoTable::GetItem(CDbCursor& cursor)
{
    ...
    item.iPaiSheTimeSec = cursor.GetLong("paishetime");
    ...
    item.iAddTimeSec = cursor.GetLong("addtime");
...
{
    Server::CTools::SecInfo(item.iPaiSheTimeSec, &(item.iYear), &(item.iMonth), &(item.iDay));
    CDbDriver* pDbDriver = LOCKMEDIADB
    BOOL bRet = pDbDriver->ExecuteSQL("insert into tbl_mediainfo(paishetime, year, month, day, md5num, weizhi, location, meititype, meitisize, devicename,width,height,duration,mediaaddr,addtime,hasextra,pinnedtime,commentshort,comment)values \
    				...
    				item.iPaiSheTimeSec, item.iYear, item.iMonth, item.iDay, item.strMd5Num.c_str(), item.strWeiZhi.c_str(), item.strLocation.c_str(),
                    item.iMediaType, item.iMeiTiSize, item.strDeviceName.c_str(), item.iWidth, 
                    item.iHeight, item.iDuration, item.strAddr.c_str(), item.iAddTimeSec, item.iHasExtra, item.strCommentShort.c_str(), item.strComment.c_str());
    item.iID = pDbDriver->GetLastInsertID();
     UNLOCKMEDIADB
     //计数加 1
     CMediaJishuTable mediaJiShu;
     mediaJiShu.Increase(item.iPaiSheTimeSec, item.iMediaType, item.strDeviceName.c_str());
     //备注也要加上去
...
int CMediaInfoTable::GetItemPaiTime(int iID)
{
    ...
    return mediaInfoItem.iPaiSheTimeSec;
}
...
string CMediaInfoTable::GetItemIDSql(int iID, BOOL bNext, string strOtype, string strDevNames, int iGid, int iYear, int iMonth, int iDay, string strLocation)
{
    ...
    {
        long iAddTimeSec = mediaInfoItem.iAddTimeSec;
        int iCurTime = Server::CTools::CurTimeSec();
        int iLimitTime = iCurTime - 7*24*60*60;
        //最近
        if(TRUE == bNext)
...
string CMediaInfoTable::GetItem(int iID, BOOL& bGet)
{
    ...
    jsonItem["paitime"] = item.iPaiSheTimeSec;
    jsonItem["weizhi"] = item.strWeiZhi;
    jsonItem["location"] = item.strLocation;
    jsonItem["addtime"] = item.iAddTimeSec;
...
BOOL CMediaInfoTable::UpdateGpsWeiZhi(string strGps, string strLocation)
{
    CDbDriver* pDbDriver = LOCKMEDIADB
    BOOL bRet = pDbDriver->ExecuteSQL("update tbl_mediainfo set location='%s' where weizhi='%s' and location=''", strLocation.c_str(), strGps.c_str());
    UNLOCKMEDIADB
    return bRet;
}
BOOL CMediaInfoTable::UpdateGpsWeiZhi(int iItemid, string strGps, string strLocation)
{
    CDbDriver* pDbDriver = LOCKMEDIADB
    BOOL bRet = pDbDriver->ExecuteSQL("update tbl_mediainfo set location='%s' where weizhi='%s' and id='%d'", strLocation.c_str(), strGps.c_str(), iItemid);
    UNLOCKMEDIADB
    return bRet;
}
