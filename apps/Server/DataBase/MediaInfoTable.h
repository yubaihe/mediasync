...
struct MediaInfoItem
{
    ...
    int iID;               //ID
    long iPaiSheTimeSec;       //拍摄时间
    long iAddTimeSec;          //添加时间
    uint16_t iYear;              //年
    uint8_t iMonth;             //月
    uint8_t iDay;             //日
...
public:
    ...
    */
    ...
    static BOOL UpdateGpsWeiZhi(string strGps, string strLocation);
    static BOOL UpdateGpsWeiZhi(int iItemid, string strGps, string strLocation);
    // 统计指定位置年月日的图片和视频总数量
