...
void CMessageHandler::OnMessage(const char* pMsg, char* pRet)
{
    ...
    if(0 == strlen(pMsg))
    {
        ...
        LibDbus_SendSync(DBUS_MEDIAPARSE, 0x5007, (const char*)szMessage, strlen(szMessage) + 1, pRet, &iDbusLen);
        return;
    }
    int iMsgLen = 0;
    if(NULL != pMsg)
    ...
    BOOL bRet = LibDbus_SendSync(DBUS_MEDIAPARSE, 0x5007, (const char*)pMsg, iMsgLen, pRet, &iDbusLen);
    if(FALSE == bRet)
    {
