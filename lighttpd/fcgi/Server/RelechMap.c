...
unsigned int RelechMap_BkdrHash(RelechMap* pMap, const char *str)
{
...
int RelechMap_SetAt(RelechMap* pMap, const char* pKey, void* pValue, int iValueLen)
{
	if(pMap->iMapSize <= 0)
	{
...
void* RelechMap_LookUp(RelechMap* pMap, const char* pKey)
{
	if(pMap->iMapSize <= 0)
	{
	...
	int iPos = RelechMap_BkdrHash(pMap, (const char*)pKey)%pMap->iMapSize;
	int iIntLen = sizeof(int);
	if (NULL == pMap->pMap[iPos])
	{
		return NULL;
...
int RelechMap_Earse(RelechMap* pMap, const char* pKey)
{
	if(pMap->iMapSize <= 0)
	{
		return -1;
	...
	if (NULL == pVector->pNext)
	{
		while(1)
		{
			...
			if (NULL != pMap->pMap[pVector->iIndex])
			{
				...
				pVector->pValue = pVector->pKey + pVector->iKeyLen;
