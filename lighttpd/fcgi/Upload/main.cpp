...
void ResponseJson(const char* pszBuffer, FCGX_Stream* pStream)
{
	int iSendLen = strlen(pszBuffer) + 100;
	char* pszRetBuffer = (char*) malloc(iSendLen);
	memset(pszRetBuffer, 0, iSendLen);
	strcpy(pszRetBuffer, "Status: 200 OK\r\nContent-Type: text/html\r\n\r\n");
	strcat(pszRetBuffer, pszBuffer);
	strcat(pszRetBuffer, "\r\n");
	FCGX_PutStr(pszRetBuffer, strlen(pszRetBuffer), pStream);
	free(pszRetBuffer);
	pszRetBuffer = NULL;
}
int main(int argc, char** argv) 
{
	...
	FCGX_Request cgi;
	if (FCGX_InitRequest(&cgi, 0, 0) != 0)
	{
		return 2;
	}
	std::string strBaseDir;
	while (FCGX_Accept_r(&cgi) == 0)
	{
		const char* pszMethod = FCGX_GetParam("REQUEST_METHOD", cgi.envp);
        const char* pszContentType = FCGX_GetParam("CONTENT_TYPE", cgi.envp);
		if (NULL == pszMethod || strcmp(pszMethod, "POST") != 0)
		{
			ResponseJson("405 Method Not Allowed", cgi.out);
            FCGX_Finish_r(&cgi);
            continue;
        }
		const char* pszBoundaryPtr = strstr(pszContentType, "boundary=");
        if (NULL == pszBoundaryPtr)
		{
			ResponseJson("400 Bad Request", cgi.out);
            FCGX_Finish_r(&cgi);
            continue;
        }
		pszBoundaryPtr += strlen("boundary=");
		std::string strBoundary(pszBoundaryPtr, strcspn(pszBoundaryPtr, "\r\n ;"));
		if (TRUE == strBaseDir.empty())
		{
            strBaseDir = GetStoreAddr();
            if (TRUE == strBaseDir.empty())
			{
				ResponseJson("500 Internal Server Error", cgi.out);
                //syslog(LOG_ERR, "Failed to get store address");
                //FCGX_PutS("Status: 500 Internal Server Error\r\n\r\n", cgi.out);
                FCGX_Finish_r(&cgi);
                continue;
            }
            if (strBaseDir.back() != '/') strBaseDir += '/';
        }
		CMultipartParser parser(strBoundary, strBaseDir);
        char szBuffer[8192] = {0};
        int iRead;
        bool bSuccess = TRUE;
        while ((iRead = FCGX_GetStr(szBuffer, sizeof(szBuffer), cgi.in)) > 0)
		{
            if (FALSE == parser.FeedData(szBuffer, iRead))
			{
                bSuccess = FALSE;
                break;
            }
        }
        if (TRUE == bSuccess && parser.IsFinished())
		{
			ResponseJson("success", cgi.out);
