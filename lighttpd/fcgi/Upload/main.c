...
void InitParam()
{
    ...
    char* pszFileDir = (char*)json_object_get_string(json_object_object_get(pServerObj, "filedir"));
    strcpy(FILEDIR, pszFileDir);
...
BOOL PraseFile(char* pszBound, char* pszDestFold, FCGX_Stream* pStream)
{
	//--pszboundaryIdentify\r\n
	//\r\n--pszboundaryIdentify--\r\n
...
int main(int argc, char** argv) 
{
	...
	while (1) 
    {
		...
		if(!strncmp("POST", FCGX_GetParam("REQUEST_METHOD", cgi.envp), strlen("POST")))
        {
            ...
            BOOL bRet = PraseFile(szBoundary, FILEDIR, cgi.in);
            if(bRet)
            {
