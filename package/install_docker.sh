#!/bin/sh
#与default的区别：
#1. docker内置了samba，只要提交了配置文件然后在修改 /etc/samba/smb.conf 增加 include = /home/relech/mediasync/samba/lib/smbmedia.conf
#2. ntfs-3g ntp
PWD=$(pwd)/package
...
#把所有的应用拷贝进来
mkdir -p $PWD/install/apps/MediaParse
cp -R $ROOTDIR/apps/MediaParse $PWD/install/apps/ -R
mkdir -p $PWD/install/apps/Server
cp -R $ROOTDIR/apps/Server $PWD/install/apps/ -R
cp $PWD/startup/docker.sh $PWD/install/start.sh
chmod +x $PWD/install/start.sh
#第三方可执行文件
